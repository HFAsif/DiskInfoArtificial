...
namespace DiskInfoArtificial
{
    ...
    using static Hlp;
    using System.Text;
    using DiskInfoArtificial.Library;
...
//                {
//                    //var ssee = queryObj.GetText(TextFormat.Mof);
...
//        }
        ...
        public override bool InternalStartup()
        {
            ...
            bool detectUASPdisks;
            Unsafe.SkipInit(out detectUASPdisks);
            bool detectUSBMemory;
            Unsafe.SkipInit(out detectUSBMemory);

            

            ManagementObjectSearcher moSearcher = new ManagementObjectSearcher("SELECT * FROM Win32_DiskDrive");
            foreach (var _drive in moSearcher.Get())
            {
                string model = default, deviceId, diskSize, mediaType = default, interfaceTypeWmi = default, pnpDeviceId = default, firmware = default;
                int physicalDriveId = -1, scsiPort = -1, scsiTargetId = -1, scsiBus = -1;

                uint siliconImageType = 0;

                #region MyRegion
                if (_drive["Size"].ToString() is String _Size && !string.IsNullOrEmpty(_Size))
                {
                    diskSize = _Size;
                }
                if (_drive["DeviceID"].ToString() is String _DeviceID && !string.IsNullOrEmpty(_DeviceID))
                {
                    deviceId = _DeviceID;
                    ...
                    if (result >= 10)
                    {
                        //physicalDriveId = int.Parse(deviceId.Substring(deviceId.Length - 2));
                    ...
                    else
                    {
                        ...
                        //physicalDriveId = int.Parse(deviceId.Substring(deviceId.Length - 1));
                    }
                }
                //((ManagedTDK.Device)(new System.Collections.Generic.Mscorlib_CollectionDebugView<Interfaces.IDevice>(dev.devices).Items[1])).TDKMarketingString

                if (_drive["Model"].ToString() is String _Model && !string.IsNullOrEmpty(_Model))
                {
                    model = _Model;
                    //if (model == "VK001920GWTTC")
                    //{
                    //    //var ssss = _drive.GetText(TextFormat.Mof);
                    //}

                    //Debug.WriteLine(model.ToString());
                }
                if (_drive["FirmwareRevision"].ToString() is String _FirmwareRevision && !string.IsNullOrEmpty(_FirmwareRevision))
                {
                    firmware = _FirmwareRevision;
                }
                if (_drive["SCSIPort"].ToString() is String _SCSIPort && !string.IsNullOrEmpty(_SCSIPort))
                {
                    scsiPort = int.Parse(_SCSIPort);
                }
                if (_drive["SCSITargetId"].ToString() is String _SCSITargetId && !string.IsNullOrEmpty(_SCSITargetId))
                {
                    scsiTargetId = int.Parse(_SCSITargetId);
                }
                if (_drive["SCSIBus"].ToString() is String _SCSIBus && !string.IsNullOrEmpty(_SCSIBus))
                {
                    scsiBus = int.Parse(_SCSIBus);
                }
                if (_drive["MediaType"].ToString() is String _MediaType && !string.IsNullOrEmpty(_MediaType))
                {
                ...
                if (_drive["InterfaceType"].ToString() is String _InterfaceType && !string.IsNullOrEmpty(_InterfaceType))
                {
                    interfaceTypeWmi = _InterfaceType;
                }
                if (_drive["PNPDeviceID"].ToString() is String _PNPDeviceID && !string.IsNullOrEmpty(_PNPDeviceID))
                {
                ...
                if (!flagBlackList)
                {
                    ...
                    {
                        //Debug.WriteLine(("INTERFACE_TYPE_IEEE1394"));
                    ...
                    {
                        //Debug.WriteLine(("INTERFACE_TYPE_USB"));
                    ...
                    {
                        //Debug.WriteLine(("INTERFACE_TYPE_NVME"));
        ...
        bool CheckSmartAttributeCorrect(ref ATA_SMART_INFO asi1, ref ATA_SMART_INFO asi2)
        {
            ...
            {
                Debug.WriteLine(("asi1.AttributeCount != asi2.AttributeCount"));
                return false;
            }
            ...
            for (uint i = 0; i < asi1.AttributeCount; i++)
            {
                ...
                {
                    Debug.WriteLine(("asi1.Attribute[i].Id != asi2.Attribute[i].Id"));
                    return false;
                }
            }
        ...
        bool GetDiskInfo(ref ATA_SMART_INFO asi, int physicalDriveId, int scsiPort, int scsiTargetId, INTERFACE_TYPE interfaceType, COMMAND_TYPE commandType,
        ...
        bool DoIdentifyDevicePd(int physicalDriveId, byte target, ref IDENTIFY_DEVICE data)
        {
            ...
            {
                //Debug.WriteLine(("SendAtaCommandPd - IDENTIFY_DEVICE (ATA_PASS_THROUGH)"));
            ...
            if (bRet == false || cstr.IsEmpty())
            {
                ...
                //Debug.WriteLine(("SendAtaCommandPd - IDENTIFY_DEVICE"));
...
#endif
            if (!AMD_RC2_GetSmartData((uint)diskNum, asi.SmartReadData, READ_ATTRIBUTE_BUFFER_SIZE, asi.SmartReadThreshold, READ_THRESHOLD_BUFFER_SIZE))
            {
                return false;
            }
